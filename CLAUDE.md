...
- Deploy: Cloudflare Pages (project "wren-ckn", https://wren-ckn.pages.dev)
...
- The OAuth Client ID in src/oauth/gisClient.js is a PUBLIC web client ID and is
  ...
  https://wren-ckn.pages.dev (the Cloudflare Pages production origin). Do not add
  a new origin or change the flow without updating that registration, or OAuth
  login breaks in that environment.
